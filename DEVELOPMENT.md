...
上线后反馈两件事：**① 经常查到别人的水表 ② 没喊 kokomi 也去查**。两处都定位到了根因。

**② 没喊 kokomi 也去查 —— 工具描述等于给了授权**
...
"或你判断需要"被模型当成了"可以自己决定去查"，而 `kokomi-query` 是全局工具，
任何一轮都可能被调用。**只靠文案约束不可靠**，因此改成确定性硬闸：

| 层 | 做法 |
|---|---|
| 钩子 | 真正命中触发时调 `markClaim(chatKey)` 写一张凭据 |
| 工具 | `execute` 开头 `hasClaim(chatKey)` 不通过就直接拒绝，**不发起任何网络请求** |
| 描述与提示词 | 同步改成"只有当消息里出现【kokomi 查询结果】/【kokomi 指令已认领】时才调用" |

凭据（`lib/claim-window.js`）的两个设计点：

* **带 TTL（10 分钟）而不是单轮一次性**：模型可能先调 `kokomi-query`、再调
  `kokomi-send-image`，同一话题也可能连查两次；单轮一次性会把正常续查一起拦掉。
* **落盘到 `<DATA_DIR>/kokomi-claim.json`**：插件热重载会重建模块实例
  （`plugin-loader.js` 用 `import(url + '?t=…')`），纯内存凭据在重载后即丢，
  表现为"刚认领完就查不了"。落盘可跨重载，代价只是一个小文件。
* 读写失败一律静默 —— 最坏退化成"窗口内不限制"（即加闸之前的行为），不会让插件不可用。

**① 查错水表 —— 结果无法归属，错了也看不出来**

链路本身是对的（触发者 QQ → `triggerSender[chatKey]` → `user_id` 参数），
真正的问题是**产物不带归属**：

```js
const options = { note: 'kokomi 战绩图' };   // 谁的水表？看不出来
```

而 `user_id` 的兜底链是 `params.userId || triggerSender.get(chatKey) || ctx?.senderId`，
其中 `triggerSender` 按会话键存、LRU 淘汰（上限 50），进程里会话一多就可能取不到而退化。
再叠加**自动发图**：图直接进群，群里没有任何线索能发现"这不是我的"。

三处一起补：

1. **结果块标注查询主体**：`查询主体：触发者本人（QQ xxx）`；若是显式传的 `userId`
   则写「不是触发者本人」，避免模型把两者数据说混。
2. **发图备注带归属**：`note = kokomi 战绩图（查 QQ:xxx）` —— 发送队列留档里能对上人，
   这是事后排查的唯一线索。
3. **debug 日志带 `user_id`**：`kokomi 查询完成："me" user_id=xxx …`。

> 还有一类根因不在插件侧：Kokomi 的绑定存在**服务端**。若那个 `(platform, user_id)`
> 在服务端本来就绑着另一个游戏账号（被 `bind` 改写、或 QQ 号填错），服务端会照常出图、
> 插件无从分辨。此时上面第 1、2 条的标注就是发现问题的入口：看到「查询主体」与自己不符，
> 即可确认是服务端绑定不对，而不是插件查错了人。

### 1.2 自然语言 → 指令：由模型翻译，插件不做解析
...
```bash
# 离线自检（135 项）：触发判定 / 隔离断言 / URL 拼装 / 响应归一化 / 翻译能力 / 配置规整 / 文本组装
node selfcheck.mjs
...
│   ├── claim-window.js  查询凭据（防"没喊 kokomi 也去查"，带 TTL + 落盘）
