 ...
 *
 * 生命周期由 TTL + 条数上限共同约束（`prune`）：群里连发数十条 kokomi 也不会撑爆内存，
...
/**
 * 惰性创建并缓存临时目录（`<系统临时目录>/qq-agent-kokomi-helper`）。
 *
...
function ensureDir() {
  ...
  tempRoot = path.join(os.tmpdir(), 'qq-agent-kokomi-helper');
  dirReady = (async () => {
...
export async function saveImage(buffer, opts = {}) {
  ...
  const token = `k${crypto.randomBytes(9).toString('hex')}${Date.now().toString(36)}`;
