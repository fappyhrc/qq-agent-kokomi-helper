 ...
 *
 * 1. 只接受 `GET`/`HEAD`，只匹配 `/kokomi/<token>` 一条路径，其余一律 404；
...
let bound = null;
/** 对外根地址，如 `http://127.0.0.1:32802`；未启动时为空串。 */
let baseUrl = '';
/** 日志出口，由 `setLog` 注入。 */
let log = () => {};
...
/**
 * 当前对外根地址。
 * @returns {string} 如 `http://127.0.0.1:32802`；未启动时为空串。
 */
 ...
 * @returns {Promise<string|null>} 成功时返回根地址（如 `http://127.0.0.1:32802`）；
 *   端口被占用等失败情况返回 `null`，**不抛错** —— 调用方据此退回 file/base64 通道，
 *   使"图片服务起不来"不至于让整个功能失效。
 * @side effect 建立监听；若已有实例且地址/端口不同，会先停掉旧实例。
 */
export async function start({ host = '127.0.0.1', port = 32802, ttlSec = 600 } = {}) {
  ...
  return new Promise((resolve) => {
    const srv = http.createServer((req, res) => {
      try {
        ...
        const m = pathname.match(/^\/kokomi\/([A-Za-z0-9_-]{8,64})$/);
        if (!m) {
    ...
    srv.listen(port, host, () => {
      ...
      log(`图片服务已就绪：${baseUrl}/kokomi/<token>`);
      resolve(baseUrl);
    });
  });
}
