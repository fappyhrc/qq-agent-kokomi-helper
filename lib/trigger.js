// kokomi-helper · 触发词识别（纯函数，可单测）
//
// 为什么需要它：判定条件是两条 AND —— 消息 **@ 了机器人本人**，且去掉 @提及 后第一个词是 kokomi。
// QQ Agent 的 OneBot 文本已经把 at 段还原成了 `@<群名片或QQ号>`，
// 所以实际收到的可能长这样：
//
//   "@老八(QQ:1000000001) kokomi me"
//   "@老八(QQ:1) kokomi ship 大和 recent 30"
//   "kokomi me"                           ← 没 @ 机器人（requireAt 打开时不认领）
//   "  kokomi   roll 日本 战列舰 10"
//   "@老八(QQ:1) 帮我看看 kokomi me"        ← ❌ 触发词不在最前
//   "@机器人(QQ:1) @kokomi me"             ← ❌ 再 @ 一次触发词也不认（@ 是"叫某人"的语法）
//
// 两条判定（AND）：
//   ① `requireAt` 打开时，消息必须 **@ 了机器人本人**（默认打开）
//   ② 去掉开头的 @提及 与空白后，**第一个词**必须是触发词
//
// ⚠️ 为什么坚持"触发词必须在最前"：群里聊到 kokomi 是常态，
//    只要含 kokomi 就触发会疯狂抢话。宁可不触发，也不能抢话。

/**
 * 提取开头的连续 @提及。
 *
 * 实现走"逐字符扫描"而不是一条大正则，原因见下面注释里记的两个真实坑：
 *   坑 1（指令丢失）：用惰性量词 /^@[^\s@]{0,64}?/ 时，"@老八(QQ:123) kokomi 大和"
 ...
 *   last    = 最后一个提及（供调用方判断"@ 了谁"，matchTrigger 不使用它）
 */
 ...
 *   command = 去掉触发词后的指令正文（不带 kokomi 前缀，正好是 Hikari 要的格式）
 */
export function matchTrigger(text, keywords = ['kokomi'], options = {}) {
  ...
  const { text: body0, mentions } = extractMentions(rawText);
  const atBot = isBotMentioned(rawText, options.selfNames || []);
  if (requireAt && !atBot) return miss('没有 @ 机器人');

  // ⚠️ 这里**故意不做**"@提及 的名字恰好等于触发词就当成头部"那种兼容（曾经有，已砍）。
  //    原因：那会让 `@机器人 @kokomi me` 也认领，而 @ 是"叫某人"的语法，
  //    用它来当触发词属于意料之外的输入，宁可不认领。
  //    注意 `@机器人 kokomi me` 并不依赖它 —— 那时 kokomi 只是普通词，
  //    由下面 head 那条判断正常处理。
  const body = body0;
